...
CREATE USER IF NOT EXISTS `wpadmin`@`%` IDENTIFIED WITH mysql_native_password BY '2qaw#WSE';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES, INDEX, ALTER, EXECUTE, CREATE VIEW, SHOW VIEW,
    CREATE ROUTINE, ALTER ROUTINE, EVENT, TRIGGER ON `wp`.* TO `wpadmin`@`%`;
CREATE USER IF NOT EXISTS `wpuser`@`%` IDENTIFIED WITH mysql_native_password BY '2qaw#WSE';
